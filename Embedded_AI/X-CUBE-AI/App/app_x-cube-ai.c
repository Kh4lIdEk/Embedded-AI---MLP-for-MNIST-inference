...
int acquire_and_process_data(ai_i8* data[])
{
  ...
  */
	...
	// Receive the raw float32 pixel buffer straight into the real input
	// buffer (data[0] == data_in_1), not into the "data" pointer-array
	// argument itself. The previous byte-by-byte reconstruction loop wrote
	// into ((uint8_t *)data), which is the address of the pointer array
	// (only AI_MNIST_IN_NUM pointers long) rather than the buffer that
	// pointer points to -- an out-of-bounds write that corrupted nearby
	// globals and left data_in_1 untouched.
...
int post_process(ai_i8* data[])
{
	...
	// Read the float32 results directly from the real output buffer
	// (data[0] == data_out_1), not from "data" itself. The previous code
	// did `uint8_t *output = data;`, which points at the pointer-array
	// argument rather than the buffer it references, so it was reading
	// whatever bytes happened to sit next to that tiny array in memory
	// instead of the actual inference output.
	float *outs = (float *)data[0];
	uint8_t outs_uint8[CLASS_NUMBER] = {0};

	for (size_t i = 0; i < CLASS_NUMBER; i++)
	{
	  // Convert each softmax probability [0,1] to a uint8_t for UART transmission
	  outs_uint8[i] = (uint8_t)(outs[i] * 255.0f);
	}

	HAL_StatusTypeDef status = HAL_UART_Transmit(&huart2, outs_uint8, sizeof(outs_uint8), TIMEOUT);
